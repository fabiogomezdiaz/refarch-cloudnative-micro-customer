...
[![Build Status](https://travis-ci.org/fabiogomezdiaz/refarch-cloudnative-micro-customer.svg?branch=master)](https://travis-ci.org/fabiogomezdiaz/refarch-cloudnative-micro-customer)
...
- `GET /micro/customer/search`
  - Return customer by username.  The caller of this API must pass a valid OAuth token with the scope `admin`.  This API is called by the [Auth Microservice](https://github.com/fabiogomezdiaz/refarch-cloudnative-auth) when authenticating a user.  A JSON object array is returned consisting of only users that match the customer username (either length 0 or 1).
...
As the APIs in this microservice as OAuth protected, the HS256 shared secret used to sign the JWT generated by the [Authorization Server](https://github.com/fabiogomezdiaz/refarch-cloudnative-auth) is needed to validate the access token provided by the caller.
...
Note that if the [Authorization Server](https://github.com/fabiogomezdiaz/refarch-cloudnative-auth) is also deployed, it must use the *same* HS256 shared secret.
...
To see the Customer app working in a more complex microservices use case, checkout our Microservice Reference Architecture Application [here](https://github.com/fabiogomezdiaz/refarch-cloudnative-kubernetes/tree/master).
...
* https://github.com/fabiogomezdiaz/refarch-cloudnative-micro-customer/graphs/contributors
